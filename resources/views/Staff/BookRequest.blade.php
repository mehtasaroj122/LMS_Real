    ...
    <script>
        ...
        class BookRequestManager {
            ...
            fetchRequests(page = 1) {
                ...
                fetch(`{{ route('staff.book-requests.data') }}?search=${encodeURIComponent(searchTerm)}&status=${status}&page=${page}`, {
                    headers: {
            ...
            refreshStats() {
                fetch(`{{ route('staff.book-requests.stats') }}`, {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
                    }
            ...
            submitCreateRequest() {
                ...
                fetch('{{ route("staff.book-requests.store") }}', {
                    method: 'POST',
                    headers: {
        ...
        window.processRequest = function(requestId, action) {
            ...
            fetch(`{{ url('staff/book-requests') }}/${requestId}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json',
