...
@push('styles')
    <style>
        ...
        /* delete button removed — not available in staff portal */
...
@section('content')
        ...
        <div class="p-3 mb-3 card">
            <div class="flex flex-col items-start justify-between gap-2 md:flex-row md:items-center">
                <div class="flex items-center gap-2">
                    <div class="relative">
                        <input type="text" id="searchInput" placeholder="Search students..."
                            class="w-full py-2 pl-10 pr-4 transition bg-transparent border border-gray-300 rounded-lg dark:border-gray-600 text-primary focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent md:w-64">
                        <i data-lucide="search" class="w-4 h-4 text-gray-400 absolute left-3 top-2.5"></i>
                    </div>
                    <select id="departmentFilter"
                        class="px-3 py-2 transition bg-transparent border border-gray-300 rounded-lg filter-select dark:border-gray-600 text-primary focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <button id="addStudentBtn"
                    class="flex items-center gap-2 px-3 py-1 font-medium text-white transition bg-blue-600 rounded-lg hover:bg-blue-700">
    ...
    <div id="addStudentModal"
        ...
        <div
            ...
            <form action="{{ route('staff.students.store') }}" method="POST" id="addStudentForm">
                @csrf
    ...
    <script>
        class StudentManager {
            ...
            fetchStudents(page = 1) {
                ...
                fetch(`{{ route('staff.students.data') }}?search=${encodeURIComponent(searchTerm)}&department=${department}&page=${page}`, {
                        headers: {
            ...
            refreshStats() {
                fetch(`{{ route('staff.students.stats') }}`, {
                        headers: {
                            'Accept': 'application/json',
                            'X-Requested-With': 'XMLHttpRequest',
                        }
        ...
        // Deletion removed: staff users cannot delete students from this UI
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (form) {
                ...
                function validateAddStudentForm() {
                    ...
                    const roll_no = document.getElementById('modal_roll_no')?.value.trim();
                    const department = document.getElementById('modal_department_id')?.value;
                    ...
                    if (!email) {
                        ...
                        const re = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;
                        if (!re.test(email)) errors.email = 'Enter a valid email address.';
                    }
                    if (!phone) {
                    ...
                    if (!roll_no) errors.roll_no = 'Roll number is required.';
                    if (!department || department === '') errors.department_id = 'Select a department.';
                ...
                form.addEventListener('submit', (e) => {
                    ...
                    const clientErrors = validateAddStudentForm();
                    if (Object.keys(clientErrors).length > 0) {
                        Object.keys(clientErrors).forEach(field => {
                            ...
                            if (input) {
                                ...
                                if (errorEl) {
                                    errorEl.textContent = clientErrors[field];
                    ...
                    fetch('{{ route('staff.students.store') }}', {
                            method: 'POST',
                            body: formData,
                            headers: {
                        ...
                        .then(response => {
                            if (!response.ok && response.status === 422) {
                                return response.json().then(data => { throw { validation: true, errors: data.errors }; });
                            }
                            return response.json();
                        })
                        .then(data => {
                            if (data.success) {
                                ...
                                // Build table row HTML from returned student
                                const s = data.student;
                                const statusClass = (s.user && s.user.status === 'active') ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200';
                                const deptName = (s.department && s.department.name) ? s.department.name : 'N/A';
                                const rowHtml = `
                                    <tr class="border-b border-gray-200 dark:border-gray-700" data-student-id="${s.id}">
                                        <td class="px-6 py-4"><div class="student-info"><span class="student-name">${(s.user && s.user.name) ? s.user.name : 'Unknown'}</span><span class="student-id">${s.roll_no ?? 'N/A'}</span></div></td>
                                        <td class="px-6 py-4 text-secondary">${(s.user && s.user.email) ? s.user.email : 'N/A'}</td>
                                        <td class="px-6 py-4 text-secondary">${deptName}</td>
                                        <td class="px-6 py-4 text-secondary">${s.batch ?? 'N/A'}</td>
                                        <td class="px-6 py-4"><span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${statusClass}">${(s.user && s.user.status) ? (s.user.status.charAt(0).toUpperCase() + s.user.status.slice(1)) : 'N/A'}</span></td>
                                        <td class="px-6 py-4"><div class="action-buttons"><a href="/staff/students/${s.id}" class="action-btn btn-view"><i data-lucide="eye" class="w-4 h-4"></i>View</a></div></td>
                                    </tr>
                                `;
                                ...
                                // Update manager counts and UI elements
                                try {
                                    if (manager) {
                                        manager.totalRows = (manager.totalRows || 0) + 1;
                                    }
                                    ...
                                    if (s.user && s.user.status === 'active') {
                                        ...
                                        if (inactiveEl) inactiveEl.textContent = parseInt(inactiveEl.textContent || '0') + 1;
                                    }
                                } catch (err) {
                                    console.error('Error updating counts:', err);
                                }

                                // Re-init icons and accessibility handlers for new row
                                if (typeof lucide !== 'undefined') lucide.createIcons();
                                if (manager) {
                                    manager.attachEventListeners();
                                    manager.updateRowAccessibility();
                                }
